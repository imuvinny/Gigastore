...
export function ProductDetailsModal({ product, onClose, onAddToCart }: ProductDetailsModalProps) {
  ...
  const [selectedStorageData, setSelectedStorageData] = useState(selectedColorData?.storages?.[0] || null);
  const [selectedConditionData, setSelectedConditionData] = useState(selectedStorageData?.conditions?.[0] || null);
  ...
  useEffect(() => {
    if (selectedColorData) {
      // Find first storage that has any available condition, or default to first
      const availableStorage = selectedColorData.storages.find(s => s.conditions.some(c => c.available)) || selectedColorData.storages[0];
  ...
  useEffect(() => {
    if (selectedStorageData) {
      const availableCond = selectedStorageData.conditions.find(c => c.available) || selectedStorageData.conditions[0];
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <div className="w-full md:w-1/2 bg-gray-50 p-8 flex items-center justify-center border-b md:border-b-0 md:border-r border-gray-100 shrink-0 md:shrink overflow-hidden">
             <div className="relative w-full max-w-sm aspect-square md:aspect-[4/5] flex items-center justify-center max-h-[30vh] md:max-h-none">
               <motion.img
                 ...
                 src={displayImage}
                 alt={product.name}
                 className={\`w-full h-full object-contain \${!hasVariants ? 'mix-blend-multiply' : ''}\`}
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto">
            ...
            {/* Storage */}
            <div className="mb-8">
              <h3 className="text-sm font-bold text-black uppercase tracking-widest mb-1">Choose Your Storage</h3>
              <p className="text-xs text-gray-500 mb-4">Pick a size that fits your everyday needs. No slowdowns, no limits.</p>
              <div className="grid grid-cols-1 gap-3">
                {hasVariants && selectedColorData ? selectedColorData.storages.map((storage) => {
                  const isAvailable = storage.conditions.some(c => c.available);
                  return (
                  <button
                    key={storage.name}
                    ...
                    className={\`text-left p-4 rounded-xl border-2 transition-all flex justify-between \${
                      ...
                      !isAvailable ? 'border-gray-100 opacity-50 cursor-not-allowed bg-gray-50' : 'border-gray-100 hover:border-gray-200'
                    }\`}
                  >
            ...
            {/* Condition */}
            <div className="mb-8">
              <h3 className="text-sm font-bold text-black uppercase tracking-widest mb-1">Choose Your Condition</h3>
              <p className="text-xs text-gray-500 mb-4">Every device passes our 90+ Point Inspection and ships fully functional.</p>
              <div className="grid grid-cols-1 gap-3">
                {hasVariants && selectedStorageData ? selectedStorageData.conditions.map((cond) => (
                  <button
                    key={cond.name}
                    ...
                    className={\`text-left p-4 rounded-xl border-2 transition-all flex justify-between items-start \${
                      ...
                      !cond.available ? 'border-gray-100 opacity-50 cursor-not-allowed bg-gray-50' : 'border-gray-100 hover:border-gray-200'
                    }\`}
                  >
                    <div>
...
`;
fs.writeFileSync('src/components/ProductDetailsModal.tsx', content);
